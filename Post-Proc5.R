...
for (qry in qries){
  ...
  ifelse(qry=="CO2 emissions by sector (no bio).proj",
          ...
          ifelse(qry=="nonCO2 emissions by resource production.proj",
                 ...
                 ifelse(qry=="nonCO2 emissions by sector.proj",
                        ...
                        ifelse(qry=="Land Use Change Emission (future).proj",
                               ...
                               ifelse(qry=="aggregated land allocation.proj",
                                      ...
                                      ifelse(qry=="aggregated land allocation.proj",
                                             ...
                                             ifelse(qry=="prices of all markets.proj",
                                                   ...
                                                   ifelse(qry=="total final energy by aggregate sector",
                                                          {FinalEnergy<- prj$scenario$`total final energy by aggregate sector` %>%
                                                            group_by(scenario, region, experiment, old_scen_name,Units, year) %>%
                                                            summarize(value=sum(value))%>%mutate(Metric="Final Energy");},
                                                          ifelse(qry=="total final energy by aggregate sector",
                                                                 {FinalEnergy<- prj$`elec prices by sector`%>%  filter(year %in% c(1990,2005,2010,2015,2020,2025,2030,2035,2040,2045,2050,2065,2080,2095));
                                                                 electPrice<-ElecPrice%>%  filter(fuel %in% c("elect_td_bld")) %>%group_by(scenario, region, experiment, old_scen_name, year);
                                                                 trnPrice<-ElecPrice %>%  filter(fuel %in% c("elect_td_trn"))%>%group_by(scenario, region, experiment, old_scen_name,  year);
                                                                 indPrice<-ElecPrice%>%  filter(fuel %in% c("elect_td_ind")) %>%group_by(scenario, region, experiment, old_scen_name, year);
...
elecPrice$value<-(electPrice$value*bldEnergy$value*(10**3))+(trnPrice$value*trnEnergy$value*(10**3))+(indEnergy$value*indPrice$value*(10**3))
